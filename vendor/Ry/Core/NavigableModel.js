...
class NavigableModel extends Component
{
        ...
        this.props.store.subscribe(()=>{
            ...
            if(state.type===this.model) {
                if(state.row && state.row.id) {
                    ...
                    if(this.progressive && state._url) {
                        let endpoint = state._url.replace(/[&\?]page=\d*/, '').replace(/[&\?]json[^&]*/, '')
                    ...
                    this.setState({
                        ...
                        page:state.data.current_page?state.data.current_page:1,
    ...
    remove(index, id, callbacks={success:null,error:null}) {
        const dis = this
        if(this.xhr)
            this.xhr.abort();
    ...
    builPaginationFromQuery(page) {
        ...
        queries.page = page
        queries.json = true
        return this.endpoint + (/\?/.test(this.endpoint)?'&':'?') + qs.stringify(queries)
    }
