...
import {COLORS} from './Pie';

class Pie extends Component
{
    constructor(props) {
        super(props)
        const items = [...this.props.data.data.filter(it=>it.quantity>0)]
        items.sort((a, b)=>{
            ...
            <div className="footer">
                <div className="container-fluid">
                    {this.data.map((item, key)=>{
                        ...
                        return <div key={`pie-${this.props.data.id}-${key}`}>
                        <div className="chartText d-flex align-items-center justify-content-between">
                            <span>
                                <span className={`legend bg-${bgcolor}`}></span>
                                <span className="text">{item.title}</span>
                            </span>
                            <span className="legendBold" style={{minWidth:56, textAlign:'right'}}>{this.props.format?numeral(item.quantity).format(this.props.format):(numeral(item.quantity).format('0,0')+' Kg')}</span>
                        </div>
                    </div>})}
                </div>
            </div>
