...
class LineLarge extends Component
{
    componentDidMount() {
        ...
        for(var j=0; j<this.props.data.data.length; j++) {
            let data = this.props.data.data[j].orders.sort((a,b)=>a.month-b.month)
        ...
        const now = new Date();
        const kilera = ["success", "beige", "yellow", "orange", 'blue', 'grey']
        ...
        var yFormat = function(a){
            return numeral(a/1000).format('0,0.0') + 't'
        }
        ...
        for(let k=0; k<this.props.data.data.length; k++) {
            let data = []
            for(let n=0; n<12; n++) {
                let d = this.props.data.data[k].orders.find(it=>it.month===n+1)
                if(d) {
                    data.push(d)
                }
                else {
            ...
            svg.append("path")
                .data([data])
                .attr("class", `area-${kilera[k%kilera.length]}-2`)
                .attr("transform", `translate(${layout.margin.left},0)`)
                .attr("d", area);
            ...
            points.selectAll(".dot")
                ...
                .on("mouseover", function(d, b, c) { 
                    ...
                    div.html(function(){
                        return moment(new Date(year, d.month-1, 1)).format('MMM') + ' ' + year + '<br/>' + numeral(d.quantity).format('0,0.00')+'Kg'
                    })
    ...
    render() {
        return <div className={`bg-white pt-2 m-auto text-center h-100 rounded ${this.props.className}`} style={{maxWidth:480}}>
            <h6 className="font-10 text-center" style={{fontSize:"14px"}} dangerouslySetInnerHTML={{__html:this.props.data.title}}></h6>
            <div ref="line" className="vis p-2" style={{height:196, marginLeft:40}}></div>
        </div>
    }
}
