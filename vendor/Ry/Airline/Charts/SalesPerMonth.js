...
class Area extends Component
{
    componentDidMount() {
        ...
        const kilera = ['beige','info', 'yellow', 'primary', 'info'];
        for(let i=0; i<this.props.data.data.length; i++) {
            let data = []
            for(let n=0; n<12; n++) {
                let d = this.props.data.data[i].orders.find(it=>it.month===n+1)
                if(d) {
                    data.push(d)
                }
                else {
            ...
            .attr("d", area)
            tarea.attr("class", `area-${kilera[i%kilera.length]}-4`)
    ...
    render() {
        return <div className="bg-white pt-3 text-center h-100 rounded">
            <h6 className="text-center" dangerouslySetInnerHTML={{__html:this.props.data.title}}></h6>
